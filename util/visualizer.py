...
class Visualizer():
    ...
    def plot_current_evals(self, epoch, evals):
        """display the current losses on visdom display: dictionary of error labels and values
        ...
        plot_name = '_'.join(list(evals.keys()))

        if plot_name not in self.plot_data:
            self.plot_data[plot_name] = {'X': [], 'Y': [], 'legend': list(evals.keys())}

        plot_data = self.plot_data[plot_name]
        plot_id = list(self.plot_data.keys()).index(plot_name)
        
        plot_data['X'].append(epoch* 1.0)
        plot_data['Y'].append([evals[k] for k in plot_data['legend']])
        X=np.stack([np.array(plot_data['X'])] * len(plot_data['legend']), 1)
        Y=np.array(plot_data['Y'])
        # assert X.shape == Y.shape, '8348324893 X and Y should be the same shape'
        try:
            self.vis.line(
                X=np.stack([np.array(plot_data['X'])] * len(plot_data['legend']), 1),
                Y=np.array(plot_data['Y']),
