...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'fgcut':
        net = FgCUT(input_nc, ngf, ab_pos=True)
    elif netG == 'swin_unit':
        net = SwinUnit()
    elif netG == 'gru':
        net = FgCUT_GRU()
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], netF_nc=256, load_path=None):
    if netF == 'global_pool':
        net = PoolingF()
    elif netF == 'reshape':
        net = ReshapeF()
        ...
        net = PatchSampleF(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=netF_nc, load_path=load_path)
...
class PatchSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[], load_path=None):
    ...
    def create_mlp(self, feats):
        ...
        if isinstance(self.load_path, str):
            state_dict = torch.load(self.load_path, map_location='cuda:'+str(self.gpu_ids[0]))
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if num_patches > 0:
                ...
                x_sample = feat_reshape[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1])  #[num_patches, c]
            else:
...
class SPyNet(nn.Module):
    ...
    def __init__(self, pretrained):
        ...
        if isinstance(pretrained, str):
            # .load_state_dict(torch.load(pretrained))
        ...
        self.register_buffer(#定义参数，在模型训练时不会更新
            'mean',
